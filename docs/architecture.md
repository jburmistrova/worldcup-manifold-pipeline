...
A market is "well-calibrated" if, among all the events it assigned a 70% probability to, roughly 70% of them actually happened. That's a concrete, testable claim: when the market said 70%, was it right about 70% of the time, and not, say, 50% or 90%? Sharper than just asking whether the favorite usually won.
...
**What it is:** a distributed data-processing engine. The useful mental model: a DataFrame API designed for exactly this shape of problem. Group data by key (here, by market), order it by time, and compute rolling/windowed aggregations across every group in parallel, instead of writing Python loops.
...
**What it does in this project:** packages ingest + Spark + dbt as containers and runs them as a **Job**, not a Deployment or CronJob. That distinction is deliberate and worth being able to explain: a Deployment is for long-running services that should always be up; a CronJob is for something that repeats on a schedule; a **Job** is for something that runs to completion once and stops. Exactly this, since the tournament is over and this is a one-time backfill, not live polling. If a pod crashes mid-run, the Job controller retries it automatically up to `backoffLimit`. That's retry semantics you get from the platform instead of hand-rolling a retry loop in Python.
...
Ingestion, Spark, and all of dbt (staging, `int_market_implied_probability`, both marts) are done and verified, including a real answer to the problem statement. See [docs/results.md](results.md) for the actual calibration finding and chart. Several real bugs found and fixed along the way (search-result truncation, a read-timeout under rapid API calls, ingest scripts silently depending on directories nobody remembered creating). See [docs/decisions/](decisions/) and `lessons_learned.md`. Kubernetes is built, deployed, and verified end-to-end: `docker build` -> `minikube image load` -> `kubectl apply` against a real minikube cluster, watched via `kubectl logs -f` through the full ingest -> Spark -> dbt -> analysis run, `kubectl get jobs` confirming `Complete`. Along the way, also closed the gap between "runs correctly" and standard dbt/DE production practice: an incremental model, `dbt_utils`, enforced model contracts, a shared macro, and CI running the real pipeline against a small fixture on every push. See `docs/data_engineering_best_practices.md` for the full list, all verified working, not just configured.
