...
Ingestion (markets, market answers, bets — 400,207 bet records) and the Spark extract/flatten step are both done and verified, producing typed Parquet for all three datasets. 6 ADRs logged, including two written after real mistakes (a duplicate-data bug, and a raw-data-mutation mistake made while fixing it) rather than only the ones that went smoothly — see [docs/decisions/](decisions/). Next: dbt project setup against DuckDB.
