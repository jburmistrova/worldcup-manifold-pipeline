...
Ingestion, the Spark extract/flatten step, and dbt's staging layer (all 3 models) are done and verified. `int_market_implied_probability` — the intermediate model doing the actual reconstruction work (running VWAP, repricing-jump ranking, correctly partitioned by `(market_id, answer_id)` so multi-choice answers don't get mixed together) — is also built and passing tests. Found and fixed a third real-trade filtering case while building it: zero-amount "seeding" events that broke the VWAP with a `0/0` division, caught by a bounds test rather than by inspection. 6 ADRs logged, including two written after real mistakes rather than only the ones that went smoothly — see [docs/decisions/](decisions/). Next: `mart_market_efficiency` — the core-scope mart that actually answers the problem statement.
